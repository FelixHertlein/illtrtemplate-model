...
class Attention(nn.Module):
    ...
    def forward(self, x, task_embed=None, level=0):
        ...
        qkv = (
            self.qkv(x)
            .reshape(N, L, 3, self.num_heads, D // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )
        q, k, v = (
            qkv[0],
...
class ResBlock(nn.Module):
    def __init__(self, channels):
        ...
        self.conv1 = nn.Conv2d(
            channels, channels, kernel_size=5, stride=1, padding=2, bias=False
        )
...
class PatchEmbed(nn.Module):
    ...
    Note: function is identical to the following code:
        einops.rearrange(x, "n c (h p0) (w p1) -> n (w h) (c p0 p1)", p0=4, p1=4)
...
class IllTr_Net(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, (128 // patch_size) ** 2, self.embed_dim)
        )
